...
def encode_file(
    ...
    with open(input_path,"r",encoding="utf-8") as f_in, open(output_path, "wb") as f_out:
        ...
        if buffer:
            ...
            arr.tofile(f_out)
            total_tokens += len(buffer)
    print(f"Finished: {total_tokens:,} tokens")
...
def main():
    ...
    print(f"Tokenizer vocab size: "f"{len(tokenizer.vocab)}")

    encode_file(TRAIN_TEXT,TRAIN_BIN,tokenizer)
    encode_file(VAL_TEXT,VAL_BIN,tokenizer)
